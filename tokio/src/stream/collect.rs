...
impl sealed::FromStreamPriv<String> for String {
    ...
    fn extend(collection: &mut String, item: String) -> bool {
        if collection.is_empty() {
            mem::replace(collection, item);
        } else {
...
impl<T> sealed::FromStreamPriv<T> for Box<[T]> {
    ...
    fn finalize(collection: &mut Vec<T>) -> Box<[T]> {
        <Vec<T> as sealed::FromStreamPriv<T>>::finalize(collection)
            .into_boxed_slice()
    }
}
